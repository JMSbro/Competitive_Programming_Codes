...
int main()
{
    ...
    set<int>:: iterator its = s.find(978); ///Gives iterator its the element inside the find() function, if it is found ; otherwise returns s.end() ,meaning the size of the set.
    cout << *its << " is the its value" << endl;
	/// Erasing front element in set

	s = { 1, 2, 3, 4, 5, 6 };

    s.erase ( s.begin() ); /// no pointer has been used..
    cout << s.size() << endl; /// 5
