...
void solve(){
    ...
    //dbg(v); dbg(m);
    int ocnt = 0, eocnt = 0, eecnt = 0;
    for(auto u : m){
        if(u.s & 1) ocnt++;
        else if(!(u.s & 1) && ((u.s /2) & 1)) eocnt++;
        else eecnt++;
    }
    //dbg(ocnt, eocnt, eecnt);
    //if(eecnt & 1) eecnt--;
    if((eecnt & 1) && ocnt == 0) eecnt--;
    if(m.size() == 1){
        if((m[v[0]] / 2) & 1) cout << 2 << endl;
        else cout << 0 << endl;
        return;
    }
    cout<< (eocnt * 2) + ocnt + (eecnt * 2) << endl;
