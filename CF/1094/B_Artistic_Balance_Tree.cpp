...
void solve(){
    ...
    if(evencnt != 0){
        ...
        if(evencnt == 0) evenidx = 1;
    }

    if(oddidx == -1) oddidx = 0;
    if(evenidx == -1) evenidx = 0;
    //dbg(oddidx, evenidx);

    for(int i = oddidx; i < (int)odd.size(); i++){
        sum += odd[i];
    }
    for(int i = evenidx; i < (int)even.size(); i++){
        sum += even[i];
    }
