...
void solve(){
    ...
    int diff = abs(cnto - cntn);
    if(n&1) {
        no
        return;
    }
    int needo = n/2, needn = n/2;
    if(cnto > needo) diff = abs(cnto - needo);
    else if(cntn > needn) diff = abs(cntn - needn);
    ...
    if(cnto == cntn) yes
        ...
        if(co >= diff && diff % 2 == 0) yes
        else no
    }
    else {
        if(cn >= diff && diff % 2 == 0) yes
        else no
    }
    // if(sum&1) {
    //     no
