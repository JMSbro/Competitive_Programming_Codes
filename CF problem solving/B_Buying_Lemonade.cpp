...
void solve(){
    ...
    ll tt = n * v[0], cnt = 0, tap = n * v[0],id = 1;
    while(tt < k ){
        // if(v[id] == cnt) {
        //     id++;
        //     continue;
        ...
        // cnt = v[id] - cnt;
        // id++;
        // int sub = upper_bound(all(v), cnt) - v.begin();
        // dbg(sub, cnt);

        // tap += (n * cnt) - cnt * (upper_bound(all(v), cnt - 1) - v.begin());
        // tt += (n * cnt) -  (upper_bound(all(v), cnt) - v.begin());
        // dbg(tap,tt);
